 ...
 */
static int host_keymaster_open(const hw_module_t* module, const char* name,
                                 hw_device_t** device) {
    if (strcmp(name, KEYSTORE_KEYMASTER) != 0) {
        return -EINVAL;
    }

    HostKeymasterDevice* dev = new HostKeymasterDevice(module, new keymaster::HostKeymasterClient());
    if (dev == NULL) {
        return -ENOMEM;
    }
